...
function de_scripts_and_styles() {
	...
	if ( current_user_can('edit_posts') || current_user_can( 'edit_users' ) || current_user_can( 'edit_theme_options' ) || current_user_can( 'edit_de_frontend' ) ) {
		...
		if ( file_exists( DIRECT_PATH . 'js/direct-edit-min.js' ) ) {
			...
			wp_enqueue_script( 'direct-postwrapper-editor', DIRECT_URL . 'js/direct-postwrapper-editor.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-slider', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-dialog', 'jquery-ui-button' ), DIRECT_VERSION, true );
...
<script>
	jQuery(document).ready(function() {
		directEdit(<?php echo de_json_encode( $de_global_options ); ?>, '<?php echo wp_nonce_url( admin_url( 'admin-ajax.php' ), 'de_nonce_check', '_de_nonce' ); ?>');
		<?php if ( current_user_can('edit_posts') || current_user_can( 'edit_de_frontend' ) ) { // Save button is only for users who can edit posts ?>
			jQuery('li#wp-admin-bar-save-page a').directSaveButton({success: function (m) {console.log(m)}});
		<?php } ?>
	});
</script>
	<?php
